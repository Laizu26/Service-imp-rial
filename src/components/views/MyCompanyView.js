...
const DEFAULT_CONTRACT = { type: "MERCENARIAT", contractDurationDays: "", dailyWage: "", corveeFreeDaysPerMonth: "", buyoutAmount: "", migrationLocked: false, customClauses: "" };
...
const normalizeContractTerms = (t) => ({
  ...
  dailyWage: parseFloat(t.dailyWage) || 0,
  ...
  return (
    <div className="space-y-3 text-xs bg-stone-50 border border-stone-200 rounded-xl p-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-[10px] font-black uppercase text-stone-400 mb-1 block">Salaire/jour (écus)</label>
          <input type="number" min={0} step={1} value={terms.dailyWage} onChange={(e) => set("dailyWage", e.target.value)}
            className="w-full p-2 border rounded font-mono text-sm bg-white" placeholder="0 = aucun" />
        </div>
        <div>
...
function EmployeeManagementModal({
  ...
  const [dailyWageInput, setDailyWageInput] = useState(String(empContract.dailyWage || ""));
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          {activeTab === "contract" && (
            <div className="space-y-4">
              ...
              {/* Salaire journalier */}
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-stone-400 tracking-widest block">Salaire journalier (Écus)</label>
                <div className="flex gap-2">
                  <input
                    type="number" min={0} step={0.1}
                    className="flex-1 p-2 border rounded font-mono text-sm"
                    ...
                    placeholder="0 = pas de salaire"
                  />
                  <button
                    onClick={() => onUpdateEmployeeContract({ companyId: myCompany.id, citizenId: empId, updates: { dailyWage: parseFloat(dailyWageInput) || 0 } })}
                    className="bg-emerald-700 text-white px-3 py-2 rounded text-xs font-bold uppercase hover:bg-emerald-600"
                  >
                    Sauvegarder
                  </button>
                </div>
                <p className="text-[9px] text-stone-400 italic">Versé automatiquement chaque jour depuis le trésor de l'entreprise.</p>
              </div>
                    ...
                    {myContract ? (
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-[10px]">
                          {myContract.dailyWage > 0 && (
                            <div className="bg-green-50 border border-green-100 rounded px-2 py-1.5">
                              <div className="font-black text-green-700 uppercase tracking-wide">Salaire/jour</div>
                              <div className="text-stone-600">{formatMoney(myContract.dailyWage)} par jour</div>
                            </div>
                          )}
